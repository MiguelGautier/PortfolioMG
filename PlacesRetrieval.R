...
#Variables
    ...
    for (i in 1:P) {
      ...
      ## placewebsite<-xml_text(xml_find_all(details,"//website")) 
      ## ver como traer //opening_hours//weekday_text tabulado
      ...
      placesite<-if(identical(xml_text(xml_find_all(details,"//website"))
                              ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//website"))
      
      OHMonday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[1])
                              ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[1])
            
      OHTuesday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[2])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[2])   
      
      OHWednesday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[3])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[3])
    
      OHThursday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[4])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[4])
      
      OHFriday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[5])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[5])
      
      OHSaturday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[6])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[6])
      
      OHSunday<-if(identical(xml_text(xml_find_all(details,"//weekday_text")[7])
                             ,character(0))){
        "ND"
      } else xml_text(xml_find_all(details,"//weekday_text")[7])
      
      placesdetails <- rbind(placesdetails,cbind(placename,placephone, placesite, 
                                                 OHMonday, OHTuesday, OHWednesday, 
                                                 OHThursday, OHFriday, OHSaturday,
                                                 OHSunday,placeaddress ))
      print(i)
      
  }
 
    ## Write coordinates into a csv file
    write.csv(coordinates,file = paste("Coordenadas-",keyword,"-",type,".csv")
    write.csv(placesdetails, file = "Detalles-",keyword,"-",type,".csv")
